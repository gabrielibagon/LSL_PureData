...
//the data structure for each copy of mcobject.
...
static t_class *s_lslreceive_class;
...
void* lslreceive_new(t_symbol* s, long argc, t_atom* argv)
{
	...
	t_lslreceive* x = (t_lslreceive*) pd_new(s_lslreceive_class);
	...
	if (x != NULL) {
        ...
        if (x->lsl_inlet) {
            ...
            x->out_synch = //outlets are created in order right to left, we'll index l->r :(
            x->out_count
...
//pd calls this function on loading the library.
//decalres the new classes and their properties
void lslreceive_setup(void) {  
  s_lslreceive_class = class_new(gensym("lslreceive"),  
        (t_newmethod)lslreceive_new,  
        0, sizeof(t_lslreceive),  
        CLASS_DEFAULT, (t_atomtype)0);  
  class_addbang(s_lslreceive_class, lslreceive_bang);  
}
